...
@section('userdashboard')
    <div class="sl-mainpanel">
        ...
        <div class="sl-pagebody">
            ...
            <!-- sl-page-title -->
            @include('error.error')
            <form action="{{ route('remedial.create', $nilai->id) }}" method="POST" class="form-horizontal">
                @csrf
                <div class="card pd-20 pd-sm-40">
                    <div class="form-layout">
                        <div class="row mg-b-25">
                            ...
                            <div class="col-lg-12">
                                <div class="form-group">
                                    <label class="form-control-label">Nama Ulangan:</label>
                                    <select class="form-control" name="nama_ulangan">
                                        @if ($nilai->mapel->kkm > $nilai->nilai_ulangan_harian)
                                        <option value="nilai_ulangan_harian">Ulangan Harian</option>
                                        @endif
                                        @if ($nilai->mapel->kkm > $nilai->nilai_tengah_semester)
                                            <option value="nilai_tengah_semester">Tengah Semester</option>
                                        @endif
                                        @if ($nilai->mapel->kkm > $nilai->nilai_akhir_semester)
                                            <option value="nilai_akhir_semester">Akhir Semester</option>
                                        @endif
                                    </select>
                                </div>
                            </div>
